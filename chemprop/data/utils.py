...
def weight_split(data: MoleculeDataset,
                 bias: str,
                 sizes: Tuple[float, float, float] = (0.8, 0, 0.2)) -> Tuple[MoleculeDataset, MoleculeDataset,
                                                                             MoleculeDataset]:
    """
...
def calculate_average_n_atoms(data: MoleculeDataset) -> float:
    """
    ...
    average_n_atoms = sum(n_atoms_list) / len(n_atoms_list)  # Calculate the average number of atoms
    sum_squared_diff = sum((n_atoms - average_n_atoms) ** 2 for n_atoms in n_atoms_list)
    std_n_atoms = sqrt(sum_squared_diff / len(n_atoms_list))  # Calculate the standard deviation
    return average_n_atoms, std_n_atoms
